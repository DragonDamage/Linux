...
find /* -type f -exec grep -l '127.0.0.1' {} \;  # Поиск файлов содержащих айпи-адрес 127.0.0.1
...
Вывод команды `$ top`
```bash
top - 10:06:29 up 12 days,  1:58,  2 users,  load average: 6.25, 4.99, 5.16
Tasks: 665 total,   2 running, 663 sleeping,   0 stopped,   0 zombie
%Cpu(s):  3.7 us,  7.1 sy,  0.0 ni, 87.2 id,  0.0 wa,  0.8 hi,  0.4 si,  0.9 st
MiB Mem :  64254.7 total,   1252.7 free,  20476.9 used,  43055.4 buff/cache
MiB Swap:   4096.0 total,   3974.3 free,    121.7 used.  43777.8 avail Mem
```
...
| :---: | :---: | :---: |
| top | 10:06:29 | Текущее время системы |
| up | 12 days, 1:58 | Время непрерывной работы системы (uptime) - 12 дней, 1 час, 58 минут |
| users | 2 | Количество залогиненных пользователей |
| load average | 6.25 | Средняя нагрузка за последнюю 1 минуту |
| load average | 4.99 | Средняя нагрузка за последние 5 минут |
| load average | 5.16 | Средняя нагрузка за последние 15 минут |

Вторая строка - Состояние процессов (Tasks)

`Tasks: 665 total,   2 running, 663 sleeping,   0 stopped,   0 zombie`
| Параметр | Значение | Расшифровка |
| :---: | :---: | :---: |
| total | 665 | Общее количество процессов и потоков |
| running | 2 | Активно выполняющиеся процессы |
| sleeping | 663 | Процессы в состоянии ожидания |
| stopped | 0 | Остановленные процессы (по сигналу) |
| zombie | 0 | Зомби-процессы (завершились, но не освободили ресурсы) |

Третья строка - Использование CPU в процентах

`%Cpu(s):  3.7 us,  7.1 sy,  0.0 ni, 87.2 id,  0.0 wa,  0.8 hi,  0.4 si,  0.9 st`
| Параметр | Значение | Расшифровка |
| :---: | :---: | :---: |
| us (user) | 3.7% | Процессы пользовательского пространства |
| sy (system) | 7.1% | Системные процессы ядра |
| ni (nice) | 0.0% | Процессы с измененным приоритетом (nice) |
| id (idle) | 87.2% | Простой процессора |
| wa (iowait) | 0.0% | Ожидание ввода-вывода (диск, сеть) |
| hi (hardware interrupts) | 0.8% | Аппаратные прерывания |
| si (software interrupts) | 0.4% | Программные прерывания |
| st (steal time) | 0.9% | Время, украденное гипервизором (виртуализация) |

Четвертая строка - Физическая память (RAM)

`MiB Mem :  64254.7 total,   1252.7 free,  20476.9 used,  43055.4 buff/cache`
| Параметр | Значение | Расшифровка |
| :---: | :---: | :---: |
| total | 64254.7 MB | Общий объем RAM (~64 ГБ) |
| free | 1252.7 MB | Полностью свободная память |
| used | 20476.9 MB | Используемая процессами память (~20 ГБ) |
| buff/cache | 43055.4 MB | Память под буферы и кэш (~43 ГБ) |

Пятая строка - Своп (Swap) и доступная память

`MiB Swap:   4096.0 total,   3974.3 free,    121.7 used.  43777.8 avail Mem`
| Параметр | Значение | Расшифровка |
| :---: | :---: | :---: |
| Swap total | 4096.0 MB | Общий объем свопа (4 ГБ) |
| Swap free | 3974.3 MB | Свободный своп |
| Swap used | 121.7 MB | Используемый своп (очень мало) |
| avail Mem | 43777.8 MB | Доступно для новых процессов (~43.8 ГБ) |
